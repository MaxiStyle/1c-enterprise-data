...
use MaxiStyle\EnterpriseData\Entities\InvoiceOut;
use DOMDocument;
use DOMElement;
...
class InvoiceOutBuilder extends DocumentBuilder implements DocumentBuilderInterface
{
    ...
    {
        if (!$invoice instanceof InvoiceOut) {
            throw new \InvalidArgumentException('Документ должен быть экземпляром InvoiceOut');
        }
        ...
        $cur = $invoice->get('currency');
        $contractCurrency->appendChild($dom->createElement('Код', $cur->get('code')));
        $contractCurrency->appendChild($dom->createElement('Наименование', $cur->get('name')));
        $contract->appendChild($contractCurrency);

        $contract->appendChild($dom->createElement('РасчетыВУсловныхЕдиницах', 'false'));
        $contract->appendChild($dom->createElement('Наименование', 'С покупателем - руб.'));
        ...
        $paymentCurrency->appendChild($dom->createElement('Код', $cur->get('code')));
        $paymentCurrency->appendChild($dom->createElement('Наименование', $cur->get('name')));
        $paymentData->appendChild($paymentCurrency);

        $paymentData->appendChild($dom->createElement('РасчетыВУсловныхЕдиницах', 'false'));
        ...
        if (!empty($invoice->get('products'))) {
            ...
            foreach ($invoice->get('products') as $product) {
                ...
                // Остальные поля
                $this->append($dom, $line, 'Код', $product->get('quantity'));
